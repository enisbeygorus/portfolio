...
test("it shows the title in the banner", () => {
  setup();
  // We expect that the title 'Hi, Iam Enis' is in the banner component
  expect(screen.getByText(/Hi, Iam Enis/i)).toBeInTheDocument();
});
...
test("can download cv when clicked on download cv button", async () => {
  ...
  // expect(downloadLink.download).toEqual('Enis-Resume.pdf');
  // expect(downloadLink.href).toEqual('/files/Enis-Resume.pdf');
